...
=head1 USAGE
  ...
  perl schema-check.pl [--schema=schemas/product.yaml] [--no-schema-listing] data1 [data2 data3]
...
=head2 C<--no-schema-listing>

By  default, the  program lists  the  full data  schema first,  before
displaying the error  messages for the JSON documents.  If this option
is activated, the schema is not displayed.
...
schema (this printing can be disabled).
