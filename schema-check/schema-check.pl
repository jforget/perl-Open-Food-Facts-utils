...
unless (@path) {
  @path = (catfile($dir_sch, $f_sch), catfile($dir_sch, $f_hid));
}
...
for my $path (@path) {
  my $new_schema = YAML::XS::Load(slurp($path));
  my $schema_file;
  ($schema_file, $dir_sch) = fileparse($path);
  $schema->{properties}{_id} = { type => 'string', description => 'Autogenerated by MongoDB' };

  find_ref_rec($new_schema, $dir_sch, $schema_file, 1, 1);

  if ($new_schema->{allOf}) {
    for my $entry (@{$new_schema->{allOf}}) {
      my $ref   = $entry->{'$ref'};
      my $dir   = $dir_sch;
      my $fname = $ref;
      my @keys  = ();
      ...
      my $subschema = YAML::XS::Load(slurp($path));
      $dir = dirname($path);
      for my $key (@keys) {
        if ($key) {
      ...
      find_ref_rec($subschema, $dir, $fname, 1, 1);
      #say JSON::PP::encode_json($subschema);
      for my $prop_name (keys %{$subschema->{properties}}) {
        #say "adding $prop_name";
        $new_schema->{properties}{$prop_name} = $subschema->{properties}{$prop_name};
      }
      for my $pattern (keys %{$subschema->{patternProperties}}) {
        #say "adding $pattern";
        $new_schema->{patternProperties}{$pattern} = $subschema->{patternProperties}{$pattern};
      }
    }
  }
  for my $prop_name (keys %{$new_schema->{properties}}) {
    #say "adding $prop_name";
    $schema->{properties}{$prop_name} = $new_schema->{properties}{$prop_name};
  }
  for my $pattern (keys %{$new_schema->{patternProperties}}) {
    #say "adding $pattern";
    $schema->{patternProperties}{$pattern} = $new_schema->{patternProperties}{$pattern};
  }
}
...
while (my $entry = shift @dyn_sch_to_do) {
  ...
  unless ($dyn_schema{$ref}) {
    ...
    find_ref_rec($subschema, $dir, $fname, $level, 1);
...
sub find_ref_rec($schema, $dir, $fname, $level, $local_top) {
  # $local_top true : the '$ref' entry is at the top of the current YAML file
  # $local_top false: the '$ref' entry is within a 'property' entry in the current YAML file
  ...
  if ($new_level > $max_depth && $local_top == 0) {
    if ($list_schema) {
