...
for my $nutriscore_data_ref0 (@test_data) {
  my $nutriscore_data_ref2 = ProductOpener::NutriscoreData2->new( %$nutriscore_data_ref0 );

  my ($score0, $grade0) = ProductOpener::Nutriscore0::compute_nutriscore_score_and_grade( $nutriscore_data_ref0, $version );
  my ($score2, $grade2) = ProductOpener::Nutriscore2::compute_nutriscore_score_and_grade( $nutriscore_data_ref2, $version );
  is($score2, $score0, "$version: score is $score2, should be $score0");
  is($grade2, $grade0, "$version: grade is $grade2, should be $grade0");
  my @missing   = ();
  my @different = ();
  for my $key (sort keys %$nutriscore_data_ref0) {
    unless ($nutriscore_data_ref2->can($key)) {
      push @missing, $key;
    }
    elsif ($nutriscore_data_ref2->$key != $nutriscore_data_ref0->{$key}) {
