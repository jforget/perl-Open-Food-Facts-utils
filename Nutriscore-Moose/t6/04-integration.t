...
for my $nutriscore_data_ref0 (@test_data) {
  my $nutriscore_data_ref6 = ProductOpener::NutriscoreData6->new( %$nutriscore_data_ref0 );

  my ($score0, $grade0) = ProductOpener::Nutriscore0::compute_nutriscore_score_and_grade( $nutriscore_data_ref0, $version );
  my ($score6, $grade6) = ProductOpener::Nutriscore6::compute_nutriscore_score_and_grade( $nutriscore_data_ref6, $version );
  is($score6, $score0, "$version: score is $score6, should be $score0");
  is($grade6, $grade0, "$version: grade is $grade6, should be $grade0");
  my @missing   = ();
  my @different = ();
  for my $key (sort keys %$nutriscore_data_ref0) {
    unless ($nutriscore_data_ref6->can($key)) {
      push @missing, $key;
    }
    elsif ($nutriscore_data_ref6->$key != $nutriscore_data_ref0->{$key}) {
