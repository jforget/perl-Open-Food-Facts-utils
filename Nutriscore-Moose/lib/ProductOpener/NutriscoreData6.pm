...
package  ProductOpener::NutriscoreData6;
use Moose;
...
sub negative_points_incr {
  my ($self, $delta) = @_;
  $self->{negative_points} += $delta;
}

sub positive_points_incr {
  my ($self, $delta) = @_;
  $self->{positive_points} += $delta;
}

sub negative_points_max_incr {
  my ($self, $delta) = @_;
  $self->{negative_points_max} += $delta;
}

sub positive_points_max_incr {
  my ($self, $delta) = @_;
  $self->{positive_points_max} += $delta;
}
...
sub proteins_incr  { my ($self, $delta) = @_; $self->{proteins}  += $delta; }
sub sugars_incr    { my ($self, $delta) = @_; $self->{sugars}    += $delta; }
sub is_cheese_incr { my ($self, $delta) = @_; $self->{is_cheese} += $delta; }
