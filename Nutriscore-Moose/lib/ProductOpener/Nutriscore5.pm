...
package ProductOpener::Nutriscore5;

use ProductOpener::PerlStandards;
use ProductOpener::NutriscoreData5;
...
sub get_value_with_one_less_negative_point_2021 ($nutriscore_data_ref, $nutrient) {
        ...
        foreach my $threshold (@{$points_thresholds_2021{$nutrient_threshold_id}}) {
                ...
                my $value_method = $nutrient . "_value";
                if (       (($nutrient eq "saturated_fat_ratio") and ($nutriscore_data_ref->$value_method >= $threshold))
                        or (($nutrient ne "saturated_fat_ratio") and ($nutriscore_data_ref->$value_method >  $threshold)))
                {
...
sub get_value_with_one_more_positive_point_2021 ($nutriscore_data_ref, $nutrient) {
        ...
        foreach my $threshold (@{$points_thresholds_2021{$nutrient_threshold_id}}) {
                my $value_method = $nutrient . "_value";
                if ($nutriscore_data_ref->$value_method < $threshold) {
...
sub compute_nutriscore_score_2021 ($nutriscore_data_ref) {
        ...
        foreach my $nutrient ("energy", $saturated_fat, "sodium", "fruits_vegetables_nuts_colza_walnut_olive_oils") {
                ...
                if (defined $nutriscore_data_ref->$nutrient) {
                        $nutriscore_data_ref->$value_method(0 + int($nutriscore_data_ref->$nutrient * 10 + 0.5) / 10);
                }
                else {
                        $nutriscore_data_ref->$value_method(0);
                }
        }
        ...
        foreach my $nutrient ("sugars", "fiber", "proteins") {
                my $value_method = $nutrient . "_value";
                if (defined $nutriscore_data_ref->$nutrient) {
                        $nutriscore_data_ref->$value_method(0 + int($nutriscore_data_ref->$nutrient * 100 + 0.5) / 100);
                }
                else {
                        $nutriscore_data_ref->$value_method(0);
                }
                ...
                "fiber", "proteins")
        {
                ...
                my $points_method = $nutrient . "_points";
                $nutriscore_data_ref->$points_method(0);
                ...
                foreach my $threshold (@{$points_thresholds_2021{$nutrient_threshold_id}}) {
                        ...
                        if (
                                (
                                        ...
                                        and ($nutriscore_data_ref->$value_method >= $threshold)
                                )
        ...
        foreach my $nutrient ("energy", "sugars", $saturated_fat, "sodium") {
                my $points_method = $nutrient . "_points";
                $nutriscore_data_ref->negative_points( $nutriscore_data_ref->negative_points + $nutriscore_data_ref->$points_method );
        }
        ...
        foreach my $nutrient (@positive_nutrients) {
                my $points_method = $nutrient . "_points";
                $nutriscore_data_ref->positive_points( $nutriscore_data_ref->positive_points + $nutriscore_data_ref->$points_method );
        }
...
sub compute_nutriscore_score_2023 ($nutriscore_data_ref) {
        ...
        foreach my $nutrient ($energy, "sugars", $saturated_fat, "salt", "fruits_vegetables_legumes", "fiber", "proteins") {
                ...
                my $max_method  = $nutrient . "_points_max";
                $nutriscore_data_ref->$max_method(scalar @{$points_thresholds_2023{$nutrient_threshold_id}});
        }
        ...
        foreach my $nutrient (@$negative_components) {
                my $points_method = $nutrient . "_points";
                my $max_method    = $nutrient . "_points_max";
                my $points     = $nutriscore_data_ref->$points_method || 0;
                my $points_max = $nutriscore_data_ref->$max_method;
                ...
                $nutriscore_data_ref->negative_points_max( $nutriscore_data_ref->negative_points_max + $points_max );
        }
        ...
        foreach my $nutrient (@{$nutriscore_data_ref->positive_nutrients}) {
                ...
                $nutriscore_data_ref->positive_points(     $nutriscore_data_ref->positive_points     + $points );
                $nutriscore_data_ref->positive_points_max( $nutriscore_data_ref->positive_points_max + $points_max );
        }
