...
package ProductOpener::Nutriscoreb;
...
sub compute_nutriscore_score_2021 ($nutriscore_data_ref) {
        ...
        foreach my $nutrient ("energy", "sugars", $saturated_fat, "sodium") {
                my $points_method = $nutrient . "_points";
                $nutriscore_data_ref->negative_points_incr(  $nutriscore_data_ref->$points_method );
        }
        ...
        foreach my $nutrient (@positive_nutrients) {
                my $points_method = $nutrient . "_points";
                $nutriscore_data_ref->positive_points_incr( $nutriscore_data_ref->$points_method );
        }
...
sub compute_nutriscore_score_2023 ($nutriscore_data_ref) {
        ...
        foreach my $nutrient (@$negative_components) {
                ...
                $nutriscore_data_ref->negative_points_max_incr( $points_max );
        }
        ...
        if ($nutriscore_data_ref->count_proteins) {
                $nutriscore_data_ref->positive_nutrients_unshift("proteins");
        }
        ...
        foreach my $nutrient (@{$nutriscore_data_ref->positive_nutrients}) {
                ...
                $nutriscore_data_ref->positive_points_incr(     $points );
                $nutriscore_data_ref->positive_points_max_incr( $points_max );
        }
        ...
        foreach my $key (keys %$nutriscore_data_ref) {
                ...
                {
                        my $nutrient = $1;
                        for my $suffix ("", "_points", "_points_max") {
                          my $method = "${nutrient}${suffix}_delete";
                          $nutriscore_data_ref->$method;
                        }
                }
        }
