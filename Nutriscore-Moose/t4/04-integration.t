...
for my $nutriscore_data_ref0 (@test_data) {
  my $nutriscore_data_ref4 = ProductOpener::NutriscoreData4->new( %$nutriscore_data_ref0 );

  my ($score0, $grade0) = ProductOpener::Nutriscore0::compute_nutriscore_score_and_grade( $nutriscore_data_ref0, $version );
  my ($score4, $grade4) = ProductOpener::Nutriscore4::compute_nutriscore_score_and_grade( $nutriscore_data_ref4, $version );
  is($score4, $score0, "$version: score is $score4, should be $score0");
  is($grade4, $grade0, "$version: grade is $grade4, should be $grade0");
  my @missing   = ();
  my @different = ();
  for my $key (sort keys %$nutriscore_data_ref0) {
    unless ($nutriscore_data_ref4->can($key)) {
      push @missing, $key;
    }
    elsif ($nutriscore_data_ref4->$key != $nutriscore_data_ref0->{$key}) {
