...
plan(tests => 12 + 2 * @test_data);

my $nutriscore_data_ref = ProductOpener::NutriscoreDataa->new(
  # Nutrients
  energy              =>  518,     # in kJ
  sugars              => 3,
  saturated_fat       => 0.7,
...
# incrementing by an explicit value
$nutriscore_data_ref->sugars_incr(   2);
$nutriscore_data_ref->proteins_incr( 2);

is($nutriscore_data_ref->sugars  , 5);
is($nutriscore_data_ref->proteins, 8.7);

# incrementng by default value 1
$nutriscore_data_ref->sugars_incr();
$nutriscore_data_ref->proteins_incr();

is($nutriscore_data_ref->sugars  , 6);
is($nutriscore_data_ref->proteins, 9.7);
...
$nutriscore_data_ref->positive_nutrients_unshift("ipsum");
my $l = $nutriscore_data_ref->positive_nutrients;
is(0 + @$l, 1, "Array 'positive nutrients' contains 1 element");
is($l->[0], "ipsum", "checking first element of Array 'positive nutrients'");

$nutriscore_data_ref->positive_nutrients_unshift("lorem");
$l = $nutriscore_data_ref->positive_nutrients;
is(0 + @$l, 2, "Array 'positive nutrients' contains 2 elements");
is($l->[0], "lorem", "checking first element of Array 'positive nutrients'");
is($l->[1], "ipsum", "checking second element of Array 'positive nutrients'");
