...
sub check($nutriscore_data_ref0, $version) {
  ...
  my ($scorea, $gradea) = ProductOpener::Nutriscorea::compute_nutriscore_score_and_grade( $nutriscore_data_refa, $version );
  is($scorea, $score0, "$version: score is $scorea, should be $score0");
  is($gradea, $grade0, "$version: grade is $gradea, should be $grade0");
  ...
  for my $key (sort keys %$nutriscore_data_ref0) {
    ...
    unless ($nutriscore_data_refa->can($key)) {
      push @missing, $key;
    }
    elsif ($nutriscore_data_refa->$key ne $nutriscore_data_ref0->{$key}) {
      # numeric test, hoping that no values are alphabetic
      push @different, sprintf("%s (%s <> %s)", $key, $nutriscore_data_refa->$key, $nutriscore_data_ref0->{$key});
    }
  }
  ...
  if ($version == 2023) {
    ...
    for my $nutrient (@{$nutriscore_data_refa->positive_nutrients}) {
      $exist{$nutrient} += 2;
    }
