...
use ProductOpener::Nutriscore1;
use ProductOpener::NutriscoreData1;
use Test::More;
...
for my $nutriscore_data_ref0 (@test_data) {
  my $nutriscore_data_ref1 = ProductOpener::NutriscoreData1->new( %$nutriscore_data_ref0 );

  my ($score0, $grade0) = ProductOpener::Nutriscore0::compute_nutriscore_score_and_grade( $nutriscore_data_ref0, $version );
  my ($score1, $grade1) = ProductOpener::Nutriscore1::compute_nutriscore_score_and_grade( $nutriscore_data_ref1, $version );
  is($score1, $score0, "$version: score is $score1, should be $score0");
  is($grade1, $grade0, "$version: grade is $grade1, should be $grade0");
  my @missing   = ();
  my @different = ();
  for my $key (sort keys %$nutriscore_data_ref0) {
    unless ($nutriscore_data_ref1->can($key)) {
      push @missing, $key;
    }
    elsif ($nutriscore_data_ref1->$key != $nutriscore_data_ref0->{$key}) {
