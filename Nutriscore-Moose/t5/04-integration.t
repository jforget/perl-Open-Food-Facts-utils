...
for my $nutriscore_data_ref0 (@test_data) {
  my $nutriscore_data_ref5 = ProductOpener::NutriscoreData5->new( %$nutriscore_data_ref0 );

  my ($score0, $grade0) = ProductOpener::Nutriscore0::compute_nutriscore_score_and_grade( $nutriscore_data_ref0, $version );
  my ($score5, $grade5) = ProductOpener::Nutriscore5::compute_nutriscore_score_and_grade( $nutriscore_data_ref5, $version );
  is($score5, $score0, "$version: score is $score5, should be $score0");
  is($grade5, $grade0, "$version: grade is $grade5, should be $grade0");
  my @missing   = ();
  my @different = ();
  for my $key (sort keys %$nutriscore_data_ref0) {
    unless ($nutriscore_data_ref5->can($key)) {
      push @missing, $key;
    }
    elsif ($nutriscore_data_ref5->$key != $nutriscore_data_ref0->{$key}) {
